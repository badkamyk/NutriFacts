...
export default function Favorites() {
    ...
    return (
        <div className="p-6 min-h-screen" role="main">
            ...
            {isLoading ? <Spinner/> :
                favorites.length > 0 ?
                    (
                        <div role="list"
                             className="grid gap-3 grid-cols-1 grid- md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 2xl:grid-cols-5 3xl:grid-cols-6">
                            {favorites?.map((recipe) => (
                                <div key={recipe.id}
                                    ...
                                    <Link href={`recipe/${recipe.id}`}>
                                        <Image className="rounded-t-lg" src={recipe.image} alt="" height={500}
                                               width={500}/>
                                    </Link>
                                    <div className="p-5">
                                        <Link href={`recipe/${recipe.id}`}>
                        ...
                        <div className="flex flex-col items-center justify-center h-full">
                            <h2 className="text-2xl font-bold text-center text-gray-700 dark:text-gray-200">You don&apos;t have
                                any favorite meals yet</h2>
                        </div>
                    )}
        </div>
    )
